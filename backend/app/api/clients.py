...
async def list_clients(
    ...
    clients = (
        db.query(Client)
        .filter(Client.workspace_id == workspace.id)
...
async def create_client(
    ...
    existing = (
        db.query(Client)
        .filter(Client.workspace_id == workspace.id, Client.name == client.name)
        .first()
    )
    if existing:
        raise HTTPException(
    ...
    db_client = Client(workspace_id=workspace.id, **client.model_dump())
    db.add(db_client)
    db.commit()
